...
export const translationKeys = Object.freeze(
  Object.keys(defaultTranslations) as TranslationKey[],
);

export const translations = Object.freeze({
  "en-US": Object.freeze(defaultTranslations),
  "es-MX": Object.freeze(
    mergeTranslations(defaultTranslations, flattenLocalization(esMX)),
  ),
...
export function formatTranslation(
  ...
  const template =
    getTranslations(locale)[key] ?? translations[DEFAULT_LOCALE][key];
  ...
  for (const [locale, catalog] of Object.entries(localizations)) {
    ...
    const missing =
      locale === DEFAULT_LOCALE
        ? translationKeys.filter((key) => !(key in flatCatalog))
        : [];
    const empty =
      locale === DEFAULT_LOCALE
        ? translationKeys.filter((key) => flatCatalog[key] === "")
        : [];
...
function parseLocalePreference(
